...
module View =
    ...
    let private statusBadge (enabled: bool) =
        if enabled then
            _span
                ...
                [ _i [ _class_ "fas fa-check-circle mr-1" ] []; Text.raw "Enabled" ]
        else
            _span
                [ _class_ "px-3 py-1 rounded text-sm font-medium bg-slate-50 text-slate-500" ]
                [ _i [ _class_ "fas fa-pause-circle mr-1" ] []; Text.raw "Disabled" ]
                  ...
                  [ infoRow
                    ...
                    infoRow
                        ...
                        (if pipeline.Enabled then
                             _span
                                 [ _class_ "text-green-600" ]
                                 [ _i [ _class_ "fas fa-check-circle mr-1" ] []; Text.raw "Active" ]
                         else
                             _span
                                 [ _class_ "text-slate-600" ]
                                 [ _i [ _class_ "fas fa-pause-circle mr-1" ] []; Text.raw "Inactive" ])
    ...
    let private stepsSection (pipelineId: int) (stepsCount: int) =
        _div
            [ _class_ "mt-6 pt-4 border-t" ]
            [ _div
                  [ _class_ "flex items-center justify-between mb-3" ]
                  [ _h3
                        [ _class_ "text-sm font-semibold text-slate-700 uppercase tracking-wide" ]
                        [ _i [ _class_ "fas fa-project-diagram mr-2" ] []
                          Text.raw "Pipeline Steps"
                          if stepsCount > 0 then
                              _span
                                  [ _class_ "text-xs font-normal text-slate-500 ml-2" ]
              ...
              if stepsCount = 0 then
                  _div
                      [ _class_ "text-center py-6 text-slate-500" ]
                  ...
                  _div
                      [ _class_ "text-center py-6 text-slate-600" ]
                  ...
                  [ _div
                        ...
                        [ _div
                              ...
                              [
                                ...
                                // Content
                                _div
                                    [ _class_ "px-6 py-4 max-h-[70vh] overflow-y-auto" ]
                                    [ _div
                                          [ _class_ "grid grid-cols-1 md:grid-cols-2 gap-6" ]
                                          [ basicInfoSection pipeline; configSection pipeline ]
    ...
    let notFound =
        _div
            [ _id_ "pipeline-details-modal"; _class_ "fixed inset-0 z-50 overflow-y-auto" ]
            [ modalBackdrop
              _div
                  [ _class_ "fixed inset-0 z-10 overflow-y-auto" ]
                  [ _div
