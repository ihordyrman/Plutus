...
module View =
    ...
    let private logo =
        _div
            [ _class_ "flex items-center justify-center mb-8" ]
            [ _div
                  [ _class_ "w-12 h-12 bg-slate-900 rounded-xl flex items-center justify-center" ]
                  [ _i [ _class_ "fas fa-chart-line text-white text-xl" ] [] ] ]

    let private formField id label inputType placeholder iconClass =
        _div
            [ _class_ "mb-4" ]
            [ _label [ _for_ id; _class_ "block text-sm font-medium text-slate-700 mb-1.5" ] [ Text.raw label ]
              _div
                  [ _class_ "relative" ]
                  [ _div
                        [ _class_ "absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none" ]
                        [ _i [ _class_ $"fas {iconClass} text-slate-400 text-sm" ] [] ]
                    _input
                        [ _type_ inputType
                          _id_ id
                          _name_ id
                          ...
                          _required_
                          _class_
                              "w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all" ] ] ]

    let private rememberMeCheckbox =
        _div
            [ _class_ "flex items-center mb-6" ]
            [ _input
                  [ _type_ "checkbox"
                    _id_ "rememberMe"
                    _name_ "rememberMe"
                    _class_ "w-4 h-4 text-slate-900 border-slate-300 rounded focus:ring-slate-900" ]
              _label
                  [ _for_ "rememberMe"; _class_ "ml-2 text-sm text-slate-600" ]
                  [ Text.raw "Remember me for 30 days" ] ]

    let private errorMessage (message: string option) =
        match message with
        | Some msg ->
            _div
                [ _class_ "mb-4 p-3 bg-red-50 border border-red-200 rounded-lg" ]
                [ _p
                      [ _class_ "text-sm text-red-600 flex items-center" ]
                      [ _i [ _class_ "fas fa-exclamation-circle mr-2" ] []; Text.raw msg ] ]
        | None -> _div [] []
    ...
    let loginForm (error: string option) =
        _html
            ...
            [ headContent
              _body
                  [ _class_ "min-h-screen bg-slate-50 flex items-center justify-center p-4" ]
                  [ _div
                        [ _class_ "w-full max-w-sm" ]
                        [ logo
                          _div
                              [ _class_ "bg-white rounded-xl shadow-sm border border-slate-200 p-8" ]
                              [ _h1
                                    [ _class_ "text-xl font-semibold text-slate-900 text-center mb-2" ]
                                    [ Text.raw "Welcome back" ]
                                _p
                                    [ _class_ "text-sm text-slate-500 text-center mb-6" ]
                                    [ Text.raw "Sign in to your account" ]
                                errorMessage error
                                _form
                                    [ _method_ "POST"; _action_ "/login" ]
                                    [ formField "username" "Username" "text" "Enter your username" "fa-user"
                                      ...
                                      _button
                                          [ _type_ "submit"
                                            _class_
                                                "w-full bg-slate-900 text-white py-2.5 px-4 rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors" ]
                                          [ Text.raw "Sign in" ] ] ]
                          _p [ _class_ "text-xs text-slate-400 text-center mt-6" ] [ Text.raw "Plutus Trading System" ] ] ] ]
                  ...
                  [ _div
                        ...
                        [ logo
                          _div
                              ...
                              [ _h1
                                    [ _class_ "text-xl font-semibold text-slate-900 text-center mb-2" ]
                                    [ Text.raw "Create Admin Account" ]
                                _p
                                    [ _class_ "text-sm text-slate-500 text-center mb-6" ]
                                    [ Text.raw "Set up your first administrator account" ]
                                errorMessage error
                                _form
                                    [ _method_ "POST"; _action_ "/setup" ]
                                    [ formField "username" "Username" "text" "Choose a username" "fa-user"
                                      ...
                                      _button
                                          [ _type_ "submit"
                                            _class_
                                                "w-full bg-slate-900 text-white py-2.5 px-4 rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors mt-2" ]
