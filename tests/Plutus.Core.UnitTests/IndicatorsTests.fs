...
[<Fact>]
let ``sma - returns None for empty series`` () =
    Assert.Equal(None, Indicators.sma 3 [])

[<Fact>]
let ``sma - returns None when series is shorter than period`` () =
    Assert.Equal(None, Indicators.sma 5 [1m; 2m; 3m])
...
[<Fact>]
let ``ema - returns None when insufficient data`` () =
    Assert.Equal(None, Indicators.ema 5 [1m; 2m; 3m])
...
[<Fact>]
let ``vwap - returns None for empty data`` () =
    Assert.Equal(None, Indicators.vwap [])

[<Fact>]
let ``vwap - returns None when total volume is zero`` () =
    Assert.Equal(None, Indicators.vwap [(100m, 0m)])
...
[<Fact>]
let ``returns - returns empty list for empty series`` () =
    Assert.Equal<decimal list>([], Indicators.returns [])
...
[<Fact>]
let ``momentum - returns None when series is too short`` () =
    Assert.Equal(None, Indicators.momentum 3 [10m; 20m])
